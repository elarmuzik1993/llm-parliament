...
class CursesLiveRenderer(DebateRenderer):
    ...
    def _redraw(self) -> None:
        ...
        # Title row
        self._safe_addstr(0, 0, "Parliament — live debate", curses.A_BOLD, width)

        # Phase header
        phase = self._current_phase or "first_reading"
        label = _PHASE_LABELS.get(phase, phase.title())
        self._safe_addstr(2, 0, f"Phase: {label}", curses.A_BOLD, width)
        ...
        if ps:
            for member in ps.members.values():
                glyph = _STATE_GLYPH.get(member.state, "?")
                duration = (
                ...
                attr = curses.A_DIM if member.state == "started" else curses.A_NORMAL
        ...
        if ps and ps.last_content:
            content_top = max(row + 1, height - 10)
            content_top = min(content_top, height - 2)
            self._safe_addstr(content_top, 0, "── Last response ──", curses.A_DIM, width)
            content_lines = ps.last_content.splitlines() or [ps.last_content]

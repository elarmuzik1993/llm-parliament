...
| **B** | What a model is called | `core/model_tiers.py::canonical_model_id`, `MODEL_TIERS`, `MODEL_ALIASES`, `presets.py` | `tests/test_model_tiers.py` checks provider-scoped identity, alias targets and known-only gaps; `tests/test_first_run_presets.py` checks preset tiers; `tests/test_parliament.py` checks runtime Speaker selection and warnings |

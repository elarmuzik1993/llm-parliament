...
| **B** | What a model is called | `MODEL_TIERS` (bare ids), `presets.py` constants, OpenRouter `vendor/slug`, Ollama `name:tag` | partly -- `get_tier` resolves an OpenRouter `vendor/slug` to its bare id (vendor prefix and routing variant dropped, Claude's `4.6` read as `4-6`), pinned by `tests/test_model_tiers.py`. A slug whose bare id is not in `MODEL_TIERS` still falls to `DEFAULT_TIER` |
